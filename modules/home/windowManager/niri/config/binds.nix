...
in {
  programs.niri.settings.binds = with config.lib.niri.actions; {
    ...
    "Mod+B".action.spawn = ["${getExe pkgs.firefox}"];
    # "Mod+R".action.spawn = ["${getExe pkgs.fuzzel}"];
    "Mod+R".action.spawn = ["${getExe inputs.anyrun.packages.${pkgs.system}.anyrun}"];
    "Mod+Shift+Q".action.spawn = ["${getExe pkgs.swaylock}"];
    "Mod+Shift+M".action.spawn = ["${getExe pkgs.wlogout}"];
